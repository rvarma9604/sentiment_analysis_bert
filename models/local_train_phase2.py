...
def permutate(embed, label, device):
...
def batch_permutate(attractor_embeds, labels, device):
    total = 0
    for embed, label in zip(attractor_embeds, labels):
        total += permutate(embed, label, device)
    return total
...
def train(model, train_loader, optimizer, device, scheduler=None):
    ...
    for batch_id, (token_id, att_mask, labels) in enumerate(train_loader):
        ...
        attractor_loss = batch_permutate(attractor_embeds, labels, device)
        loss = cls_loss + 0.001 * attractor_loss
        loss = cls_loss
...
def train_model(model, train_loader, test_loader, optimizer, device, start_epoch, max_epochs,
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        # train
        train_acc_epoch, train_loss_epoch = train(model, train_loader, optimizer, device, scheduler)
        # test
...
def main():
    ...
    parser.add_argument('--init', help='initialize mode')
